...
// Qt version < 6.2.0: to get metatypes Q_GADGET/Q_ENUMS/Q_FLAGS in namespace we have to make it look like a class
// during moc-run
...
#    endif
    ...
    */
    enum ExportPen {
        ...
        ,
        epAllowCosmetic  ///< Cosmetic pens are exported normally (e.g. in PDF exports, cosmetic pens always appear as 1
                         ///< pixel on screen, independent of viewer zoom level)
    };
    ...
    */
    enum PlottingHint {
        ...
        ,
        phFastPolylines = 0x001  ///< <tt>0x001</tt> Graph/Curve lines are drawn with a faster method. This reduces the
                                 ///< quality especially of the line segment
                                 ///<                joins, thus is most effective for pen sizes larger than 1. It is
                                 ///<                only used for solid line pens.
        ,
        phImmediateRefresh = 0x002  ///< <tt>0x002</tt> causes an immediate repaint() instead of a soft update() when
                                    ///< QCustomPlot::replot() is called with parameter \ref QCustomPlot::rpRefreshHint.
                                    ///<                This is set by default to prevent the plot from freezing on fast
                                    ///<                consecutive replots (e.g. user drags ranges with mouse).
        ,
        phCacheLabels =
    ...
    */
    enum Interaction {
        ...
        ,
        iRangeDrag = 0x001  ///< <tt>0x001</tt> Axis ranges are draggable (see \ref QCPAxisRect::setRangeDrag, \ref
        ...
        ,
        iMultiSelect = 0x004  ///< <tt>0x004</tt> The user can select multiple objects by holding the modifier set by
                              ///< \ref QCustomPlot::setMultiSelectModifier while clicking
        ,
        iSelectPlottables = 0x008  ///< <tt>0x008</tt> Plottables are selectable (e.g. graphs, curves, bars,... see
        ...
        ,
        iSelectItems = 0x040  ///< <tt>0x040</tt> Items are selectable (Rectangles, Arrows, Textitems, etc. see \ref
                              ///< QCPAbstractItem)
        ,
        iSelectOther = 0x080  ///< <tt>0x080</tt> All other objects are selectable (e.g. your own derived layerables,
                              ///< other layout elements,...)
        ,
        iSelectPlottablesBeyondAxisRect = 0x100  ///< <tt>0x100</tt> When performing plottable selection/hit tests, this
                                                 ///< flag extends the sensitive area beyond the axis rect
    };
    ...
    */
    enum SelectionRectMode {
        srmNone  ///< The selection rect is disabled, and all mouse events are forwarded to the underlying objects, e.g.
                 ///< for axis range dragging
        ,
        srmZoom  ///< When dragging the mouse, a selection rect becomes active. Upon releasing, the axes that are
                 ///< currently set as range zoom axes (\ref QCPAxisRect::setRangeZoomAxes) will have their ranges
                 ///< zoomed accordingly.
        ,
        srmSelect  ///< When dragging the mouse, a selection rect becomes active. Upon releasing, plottable data points
                   ///< that were within the selection rect are selected, if the plottable's selectability setting
                   ///< permits. (See  \ref dataselection "data selection mechanism" for details.)
        ,
        srmCustom  ///< When dragging the mouse, a selection rect becomes active. It is the programmer's responsibility
                   ///< to connect according slots to the selection rect's signals (e.g. \ref
                   ///< QCPSelectionRect::accepted) in order to process the user interaction.
    };
    ...
    */
    enum SelectionType {
        ...
        ,
        stWhole  ///< Selection behaves like \ref stMultipleDataRanges, but if there are any data points selected, the
                 ///< entire plottable is drawn as selected.
        ,
    ...
    */
    inline bool isInvalidData(double value) { return qIsNaN(value) || qIsInf(value); }
    ...
    */
    inline bool isInvalidData(double value1, double value2) { return isInvalidData(value1) || isInvalidData(value2); }
...
class QCP_LIB_DECL QCPPainter : public QPainter {
  ...
  public:
    /*!
      ...
      features/fixes/workarounds, depending on whether they are wanted on the respective output device.
    */
    enum PainterMode {
        pmDefault = 0x00  ///< <tt>0x00</tt> Default mode for painting on screen devices
        ,
        pmVectorized = 0x01  ///< <tt>0x01</tt> Mode for vectorized painting (e.g. PDF export). For example, this
                             ///< prevents some antialiasing fixes.
        ,
        pmNoCaching = 0x02  ///< <tt>0x02</tt> Mode for all sorts of exports (e.g. PNG, PDF,...). For example, this
                            ///< prevents using cached pixmap labels
        ,
        pmNonCosmetic =
            0x04  ///< <tt>0x04</tt> Turns pen widths 0 to 1, i.e. disables cosmetic pens. (A cosmetic pen is always
                  ///< drawn with width 1 pixel in the vector image/pdf viewer, independent of zoom.)
    };
    Q_ENUMS(PainterMode)
...
class QCP_LIB_DECL QCPLayer : public QObject {
  ...
  public:
    ...
    */
    enum LayerMode {
        lmLogical  ///< Layer is used only for rendering order, and shares paint buffer with all other adjacent logical
                   ///< layers.
        ,
...
class QCP_LIB_DECL QCPLayoutElement : public QCPLayerable {
  ...
  public:
    ...
    */
    enum UpdatePhase {
        upPreparation  ///< Phase used for any type of preparation that needs to be done before margin calculation and
                       ///< layout
        ,
...
class QCP_LIB_DECL QCPLayoutGrid : public QCPLayout {
  ...
  public:
    ...
    */
    enum FillOrder {
        foRowsFirst  ///< Rows are filled first, and a new element is wrapped to the next column if the row count would
                     ///< exceed \ref setWrap.
        ,
        foColumnsFirst  ///< Columns are filled first, and a new element is wrapped to the next row if the column count
                        ///< would exceed \ref setWrap.
    };
    Q_ENUMS(FillOrder)
  ...
  public:
    ...
    */
    enum EndingStyle {
        ...
        ,
        esHalfBar  ///< A bar perpendicular to the line, pointing out to only one side (to which side can be changed
                   ///< with \ref setInverted)
        ,
...
class QCPLabelPainterPrivate {
  ...
  protected:
    ...
    struct LabelData {
        ...
        QTransform transform;  // the transform about the label anchor which is at (0, 0). Does not contain final
                               // absolute x/y positioning on the plot/axis
    ...
    QColor mColor;
    int mPadding;
    double mRotation;  // this is the rotation applied uniformly to all labels, not the heterogeneous rotation in
                       // amCircularRotated mode
    ...
    // introduced virtual methods:
    virtual void drawLabelMaybeCached(QCPPainter* painter, const QFont& font, const QColor& color, const QPointF& pos,
  ...
  public:
    ...
    */
    enum TickStepStrategy {
        tssReadability  ///< A nicely readable tick step is prioritized over matching the requested number of ticks (see
                        ///< \ref setTickCount)
        ,
        tssMeetTickCount  ///< Less readable tick steps are allowed which in turn facilitates getting closer to the
...
#endif
    void setTickOrigin(double origin);  // hides base class method but calls baseclass implementation ("using" throws
...
class QCP_LIB_DECL QCPAxisTickerFixed : public QCPAxisTicker {
  ...
  public:
    ...
    */
    enum ScaleStrategy {
        ssNone  ///< Modifications are not allowed, the specified tick step is absolutely fixed. This might cause a high
                ///< tick density and overlapping labels if the axis range is zoomed out.
        ,
        ssMultiples  ///< An integer multiple of the specified tick step is allowed. The used factor follows the base
                     ///< class properties of \ref setTickStepStrategy and \ref setTickCount.
        ,
  ...
  public:
    ...
    */
    enum ScaleType {
        ...
        ,
        stLogarithmic  ///< Logarithmic scaling with correspondingly transformed axis coordinates (possibly also \ref
                       ///< setTicker to a \ref QCPAxisTickerLog instance).
    };
    Q_ENUMS(ScaleType)
    /*!
  ...
  public:
    ...
    */
    enum ScatterShape {
        ...
        ,
        ssDot  ///< \enumimage{ssDot.png} a single pixel (use \ref ssDisc or \ref ssCircle if you want a round shape
        ...
        ,
        ssDisc  ///< \enumimage{ssDisc.png} a circle which is filled with the pen's color (not the brush as with
                ///< ssCircle)
        ,
...
void QCPDataContainer<DataType>::removeBefore(double sortKey) {
    ...
    mPreallocSize += int(itEnd - it);  // don't actually delete, just add it to the preallocated block (if it gets too
                                       // large, squeeze will take care of it)
    if (mAutoSqueeze)
        performAutoSqueeze();
}
...
void QCPDataContainer<DataType>::remove(double sortKey) {
    ...
    if (it != end() && it->sortKey() == sortKey) {
        if (it == begin())
            ++mPreallocSize;  // don't actually delete, just add it to the preallocated block (if it gets too large,
...
QCPRange QCPDataContainer<DataType>::keyRange(bool& foundRange, QCP::SignDomain signDomain) {
    ...
    {
        if (DataType::sortKeyIsMainKey())  // if DataType is sorted by main key (e.g. QCPGraph, but not QCPCurve), use
                                           // faster algorithm by finding just first and last key with non-NaN value
        {
...
void QCPDataContainer<DataType>::performAutoSqueeze() {
    ...
    if (totalAlloc > 650000)  // if allocation is larger, shrink earlier with respect to total used size
    {
        shrinkPostAllocation =
            postAllocSize >
            usedSize * 1.5;  // QVector grow strategy is 2^n for static data. Watch out not to oscillate!
...
class QCP_LIB_DECL QCPAbstractPlottable : public QCPLayerable {
  ...
  public:
    ...
    // introduced virtual methods:
    virtual double selectTest(const QPointF& pos, bool onlySelectable, QVariant* details = nullptr) const
        Q_DECL_OVERRIDE = 0;  // actually introduced in QCPLayerable as non-pure, but we want to force reimplementation
                              // for plottables
  ...
  public:
    ...
    */
    enum PositionType {
        ...
        ,
        ptViewportRatio  ///< Static positioning given by a fraction of the viewport size. For example, if you call
                         ///< setCoords(0, 0), the position will be at the top left corner of the viewport/widget.
                         ///< setCoords(1, 1) will be at the bottom right corner, setCoords(0.5, 0) will be horizontally
                         ///< centered and vertically at the top of the viewport/widget, etc.
        ,
        ptAxisRectRatio  ///< Static positioning given by a fraction of the axis rect size (see \ref setAxisRect). For
                         ///< example, if you call setCoords(0, 0), the position will be at the top left corner of the
                         ///< axis rect. setCoords(1, 1) will be at the bottom right corner, setCoords(0.5, 0) will be
                         ///< horizontally centered and vertically at the top of the axis rect, etc. You can also go
                         ///< beyond the axis rect by providing negative coordinates or coordinates larger than 1.
        ,
  ...
  public:
    ...
    */
    enum RefreshPriority {
        rpImmediateRefresh  ///< Replots immediately and repaints the widget immediately by calling QWidget::repaint()
                            ///< after the replot
        ,
        rpQueuedRefresh  ///< Replots immediately, but queues the widget repaint, by calling QWidget::update() after the
                         ///< replot. This way multiple redundant widget repaints can be avoided.
        ,
        rpRefreshHint  ///< Whether to use immediate or queued refresh depends on whether the plotting hint \ref
                       ///< QCP::phImmediateRefresh is set, see \ref setPlottingHints.
        ,
        rpQueuedReplot  ///< Queues the entire replot for the next event loop iteration. This way multiple redundant
                        ///< replots can be avoided. The actual replot is then done with \ref rpRefreshHint priority.
    };
...
PlottableType* QCustomPlot::plottableAt(const QPointF& pos, bool onlySelectable, int* dataIndex) const {
    ...
    double resultDistance = mSelectionTolerance;  // only regard clicks with distances smaller than mSelectionTolerance
    ...
    foreach (QCPAbstractPlottable* plottable, mPlottables) {
        ...
        if (!currentPlottable ||
            (onlySelectable &&
             !currentPlottable
                  ->selectable()))  // we could have also passed onlySelectable to the selectTest function, but checking
                                    // here is faster, because we have access to QCPAbstractPlottable::selectable
            continue;
...
ItemType* QCustomPlot::itemAt(const QPointF& pos, bool onlySelectable) const {
    ...
    foreach (QCPAbstractItem* item, mItems) {
        ...
        if (!currentItem ||
            (onlySelectable && !currentItem->selectable()))  // we could have also passed onlySelectable to the
...
/*! \fn virtual QCPDataSelection QCPPlottableInterface1D::selectTestRect(const QRectF &rect, bool onlySelectable) const
  = 0;
...
QCPDataSelection QCPAbstractPlottable1D<DataType>::selectTestRect(const QRectF& rect, bool onlySelectable) const {
    ...
    if (DataType::sortKeyIsMainKey())  // we can assume that data is sorted by main key, so can reduce the searched key
                                       // interval:
    {
...
double QCPAbstractPlottable1D<DataType>::selectTest(const QPointF& pos, bool onlySelectable, QVariant* details) const {
    ...
    for (typename QCPDataContainer<DataType>::const_iterator it = begin; it != end; ++it) {
        ...
        if (keyRange.contains(mainKey) &&
            valueRange.contains(mainValue))  // make sure data point is inside visible range, for speedup in cases where
                                             // sort key isn't main key and we iterate over all points
        {
...
void QCPAbstractPlottable1D<DataType>::getDataSegments(QList<QCPDataRange>& selectedSegments,
    ...
    if (mSelectable == QCP::stWhole)  // stWhole selection type draws the entire plottable with selected style if
                                      // mSelection isn't empty
    {
        if (selected())
...
void QCPAbstractPlottable1D<DataType>::drawPolyline(QCPPainter* painter, const QVector<QPointF>& lineData) const {
    ...
    if (!painter->modes().testFlag(QCPPainter::pmVectorized) && qFuzzyCompare(painter->pen().widthF(), 1.0)) {
  ...
  public:
    ...
    */
    enum ColorInterpolation {
        ...
        ,
        ciHSV  ///< Color channels hue, saturation and value are linearly interpolated (The hue is interpolated over the
    ...
    */
    enum NanHandling {
        nhNone  ///< NaN data points are not explicitly handled and shouldn't occur in the data (this gives slight
    ...
    */
    enum GradientPreset {
        ...
        ,
        gpHot  ///< Continuous lightness from black over firey colors to white (suited for non-biased data
               ///< representation)
        ,
        gpCold  ///< Continuous lightness from black over icey colors to white (suited for non-biased data
                ///< representation)
        ,
        gpNight  ///< Continuous lightness from black over weak blueish colors to white (suited for non-biased data
                 ///< representation)
        ,
        ...
        ,
        gpIon  ///< Half hue spectrum from black over purple to blue and finally green (creates banding illusion but
               ///< allows more precise magnitude estimates)
        ,
        gpThermal  ///< Colors suitable for thermal imaging, ranging from dark blue over purple to orange, yellow and
                   ///< white
        ,
        gpPolar  ///< Colors suitable to emphasize polarity around the center, with blue for negative, black in the
                 ///< middle and red for positive values
        ,
        gpSpectrum  ///< An approximation of the visible light spectrum (creates banding illusion but allows more
                    ///< precise magnitude estimates)
        ,
        gpJet  ///< Hue variation similar to a spectrum, often used in numerical visualization (creates banding illusion
               ///< but allows more precise magnitude estimates)
        ,
        gpHues  ///< Full hue cycle, with highest and lowest color red (suitable for periodic data, such as angles and
                ///< phases, see \ref setPeriodic)
    };
  ...
  public:
    ...
    */
    enum BracketStyle {
        ...
        ,
        bsHalfEllipse  ///< A half ellipse is drawn. The size of the ellipse is given by the bracket width/height
                       ///< properties.
        ,
        ...
        ,
        bsUserStyle  ///< Start custom bracket styles at this index when subclassing and reimplementing \ref
                     ///< drawBracket.
    };
...
class QCP_LIB_DECL QCPGraph : public QCPAbstractPlottable1D<QCPGraphData> {
  ...
  public:
    ...
    */
    enum LineStyle {
        ...
        ,
        lsImpulse  ///< each data point is represented by a line parallel to the value axis, which reaches from the data
                   ///< point to the zero-value-line
    };
    Q_ENUMS(LineStyle)
  ...
  protected:
    ...
    QVector<QPair<QCPDataRange, QCPDataRange> > getOverlappingSegments(QVector<QCPDataRange> thisSegments,
...
class QCP_LIB_DECL QCPCurve : public QCPAbstractPlottable1D<QCPCurveData> {
  ...
  protected:
    ...
    QPointF getOptimizedPoint(int otherRegion, double otherKey, double otherValue, double key, double value,
...
class QCP_LIB_DECL QCPPolarAxisRadial : public QCPLayerable {
  ...
  public:
    ...
    */
    enum AngleReference {
        arAbsolute  ///< The axis tilt is given in absolute degrees. The zero is to the right and positive angles are
  ...
  public:
    ...
    */
    ...
    double coordToAngleRad(double coord) const {
        return mAngleRad + (coord - mRange.lower) / mRange.size() * (mRangeReversed ? -2.0 * M_PI : 2.0 * M_PI);
    }  // mention in doc that return doesn't wrap
    double angleRadToCoord(double angleRad) const {
        return mRange.lower + (angleRad - mAngleRad) / (mRangeReversed ? -2.0 * M_PI : 2.0 * M_PI) * mRange.size();
    }
    void pixelToCoord(QPointF pixelPos, double& angleCoord, double& radiusCoord) const;
    QPointF coordToPixel(double angleCoord, double radiusCoord) const;
    SelectablePart getPartAt(const QPointF& pos) const;
...
class QCP_LIB_DECL QCPPolarGrid : public QCPLayerable {
  ...
  protected:
    ...
    void drawAngularGrid(QCPPainter* painter, const QPointF& center, double radius, const QVector<QPointF>& ticksCosSin,
                         const QPen& pen);
  ...
  public:
    ...
    */
    ...
    virtual double selectTest(const QPointF& pos, bool onlySelectable, QVariant* details = 0)
        const;  // actually introduced in QCPLayerable as non-pure, but we want to force reimplementation for plottables
